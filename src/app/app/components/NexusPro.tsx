...
// Nexus Pro — the monetization surface (C-109 §7, Enterprise Workflows). A real
// Pi User-to-App payment; this also satisfies the Pi Portal "Process a
// Transaction" checklist step. ADR-007 dual-mode: Hub navigation → Mode 1 (Hub
// modal); standalone in Pi Browser → Mode 2 (direct createU2APayment). Approves
// under PI_API_KEY_NEXUS (never the default Hub key — C-12 §11).
...
const ITEM_ID = 'nexus_pro_monthly';
const MEMO    = 'TEC Nexus — Pro (1 month)';
...
export function NexusPro() {
  ...
  const handleUpgrade = async () => {
    ...
    try {
      ...
      setStatus('paying');
      const result = await createU2APayment(PRICE, MEMO, { item_id: ITEM_ID, plan: 'nexus_pro' }, internalId);
  ...
  if (status === 'success') {
    return (
      <div style={{ ...card, borderColor: `${TEC_COLORS.success}66` }}>
        <div style={{ fontSize: 15, fontWeight: 800, color: TEC_COLORS.success }}>✅ Nexus Pro active</div>
        <div style={{ fontSize: 12, color: TEC_COLORS.subtext, marginTop: 6 }}>
          Payment received. Thanks for supporting TEC Nexus 🧭.
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={card}>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 12 }}>
        <div style={{ fontSize: 15, fontWeight: 800, color: TEC_COLORS.gold }}>🧭 Nexus Pro</div>
      ...
      <div style={{ fontSize: 12, color: TEC_COLORS.subtext, marginTop: 8, lineHeight: 1.5 }}>
        ...
        coordination, and automatic rollback if anything fails. Automate complex
        activity end-to-end.
      </div>
