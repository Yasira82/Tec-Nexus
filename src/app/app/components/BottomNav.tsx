...
import { Icon, type NexusIconName } from './Icon';
import { useTranslation } from '@/lib/i18n';

export type NexusTab = 'home' | 'templates' | 'pro' | 'settings';

export function BottomNav({ active, onSelect }: { active: NexusTab; onSelect: (t: NexusTab) => void }) {
  const { t } = useTranslation();
  const ITEMS: { key: NexusTab; icon: NexusIconName; label: string }[] = [
    { key: 'home',      icon: 'workflow', label: t.nexus.nav.home      },
    { key: 'templates', icon: 'grid',     label: t.nexus.nav.templates },
    { key: 'pro',       icon: 'star',     label: t.nexus.nav.pro       },
    { key: 'settings',  icon: 'settings', label: t.nexus.nav.settings  },
  ];
  return (
    <nav style={{
